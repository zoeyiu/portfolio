...
function adjustHeight() {
  ...
  const baseHeight = "calc(130vh - 10rem)";
  const mobileHeight = contain.getAttribute('data-mobile-height');
  const smallHeight = contain.getAttribute('data-small-height');
  const extraSmallHeight = contain.getAttribute('data-extra-small-height');

  if (window.innerWidth <= 400) {
    contain.style.height = `calc(${extraSmallHeight} - 10rem)`;
  } else if (window.innerWidth <= 640) {
    contain.style.height = `calc(${smallHeight} - 10rem)`;
  } else if (window.innerWidth <= 1100) {
    contain.style.height = `calc(${mobileHeight} - 10rem)`;
  } else {
    contain.style.height = baseHeight;
  }
}
